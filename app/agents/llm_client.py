...
# Lista ordenada de modelos de respaldo en OpenRouter
...
async def call_llm_text(system_prompt: str, user_prompt: str, timeout: float = 12.0) -> Optional[str]:
    """
    Invoca el LLM con sistema de Fallback Resiliente:
    1. Proveedor Principal: Google AI Studio (API directa con GEMINI_API_KEY).
    2. Proveedor de Respaldo: Cadena iterativa de modelos en OpenRouter (OPENROUTER_API_KEY).
    3. Retorna None si todos los proveedores upstream fallan para permitir degradación elegante.
    """
    gemini_key = settings.GEMINI_API_KEY or settings.GOOGLE_API_KEY

    # -------------------------------------------------------------
    # 1. PROVEEDOR PRINCIPAL: Google AI Studio Directo
    # -------------------------------------------------------------
    if gemini_key:
        ...
        for gem_model in candidate_gemini_models:
            try:
                ...
                async with httpx.AsyncClient(timeout=timeout) as client:
                    ...
                    if resp.status_code == 200:
                        ...
                        print(f"⚠️ [Google AI Studio] HTTP {resp.status_code} en {gem_model}: {resp.text[:120]}")
            except Exception as gem_err:
                print(f"⚠️ [Google AI Studio Fallback] Error en {gem_model}: {gem_err}")

    # -------------------------------------------------------------
    # 2. PROVEEDOR DE RESPALDO: OpenRouter Fallback Chain
    # -------------------------------------------------------------
    if settings.OPENROUTER_API_KEY:
        ...
        for model_name in OPENROUTER_FALLBACK_MODELS:
            try:
                ...
                print(f"⚠️ [OpenRouter Fallback] Modelo {model_name} falló: {or_err}")

    print("🛑 [LLM Engine] Todos los proveedores de IA upstream fallaron o están saturados.")
    return None
...
async def call_llm_json(system_prompt: str, user_prompt: str, timeout: float = 14.0) -> Optional[Dict[str, Any]]:
    ...
    1. Intenta primero con Google AI Studio (response_mime_type: application/json).
    2. Itera sobre los modelos de respaldo en OpenRouter.
    3. Retorna dict estructurado o None si falla.
    """
    gemini_key = settings.GEMINI_API_KEY or settings.GOOGLE_API_KEY

    # 1. Google AI Studio Directo con modo JSON nativo
    if gemini_key:
        candidate_gemini_models = [
            "gemini-2.5-flash",
            "gemini-2.0-flash",
        ...
        for gem_model in candidate_gemini_models:
            try:
                ...
                print(f"⚠️ [Google AI Studio JSON] Error en {gem_model}: {gem_err}")
    ...
    if settings.OPENROUTER_API_KEY:
        ...
        for model_name in OPENROUTER_FALLBACK_MODELS:
            try:
                ...
                print(f"⚠️ [OpenRouter JSON] Falló modelo {model_name}: {or_err}")
