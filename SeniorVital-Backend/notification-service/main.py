"""Microservicio de notificaciones push Web Push.

Gestiona suscripciones de usuarios y envía notificaciones
push mediante la API Web Push con claves VAPID.
"""
...
class SubscribeRequest(BaseModel):
    """Solicitud para suscribir un usuario a notificaciones push."""
    user_id: str
    subscription: dict


class SendNotificationRequest(BaseModel):
    """Solicitud para enviar una notificación push a un usuario."""
    user_id: str
    title: str
    body: str
...
async def lifespan(app: FastAPI):
    """Ciclo de vida del servicio: inicializa el pool y crea la tabla si no existe."""
...
async def subscribe(req: SubscribeRequest):
    """Guarda o sobrescribe la suscripción push de un usuario.

    :param req: Datos de suscripción (endpoint y claves).
    :return: Confirmación de suscripción.
    """
    pool = await get_pool()
    async with pool.acquire() as conn:
        await conn.execute(
...
async def send_push_notification(user_id: str, title: str, body: str):
    """Envía una notificación push a un usuario en segundo plano.

    Si el endpoint devuelve 410 Gone, elimina la suscripción.

    :param user_id: ID del usuario destino.
...
async def send_notification(req: SendNotificationRequest, background_tasks: BackgroundTasks):
    """Encola el envío de una notificación push como tarea de fondo.

    :param req: Datos de la notificación (user_id, title, body).
    :param background_tasks: Gestor de tareas asíncronas de FastAPI.
    :return: Confirmación de encolado.
    """
